...
```powershell
"C:\Program Files (x86)\Inno Setup 6\ISCC.exe" /DMyAppVersion=1.4.3 finora_installer.iss
```
...
The `Windows Release` workflow runs when a `v*` tag is pushed. The tag must match `VERSION` exactly (for example, `v1.4.3`). The workflow reruns lint, security auditing, tests, builds the installer, generates a GitHub artifact attestation, and publishes the GitHub Release with SHA-256 verification material.
